...
function LeadsPage() {
  ...
  const {
    ...
    isError,
    error,
  } = useQuery({ queryKey: ["admin", "leads"], queryFn: () => list() });
        ...
        {isLoading ? (
          ...
          <div className="surface-card p-6 border border-destructive/40 rounded-xl">
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              Verifică că baza de date Cloudflare D1 (binding-ul <code>DB</code>) este configurată
              corect.
            </p>
          </div>
        ) : leads.length === 0 ? (
              ...
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium ${STATUS_TONE[l.status]}`}
                    >
                ...
                <div className="flex gap-2">
                  {(["nou", "contactat", "inchis"] as const).map((s) => (
                    <button
                      ...
                      disabled={l.status === s}
                      className={`rounded-lg border border-border bg-card px-3 py-1.5 text-xs ${l.status === s ? "opacity-50" : "hover:bg-secondary"}`}
                    >
                      {STATUS_LABELS[s]}
                    </button>
                  ))}
