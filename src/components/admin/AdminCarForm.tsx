...
type ImageRow = { url: string; alt_text?: string | null; r2_key?: string | null };
...
export function AdminCarForm({ initial }: Props) {
  ...
  async function uploadImages(files: FileList | null) {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : "Nu am putut încărca imaginile.");
    } finally {
      setUploading(false);
    }
  }
    ...
    <form onSubmit={onSubmit} className="space-y-6">
      <section className="surface-card p-6">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          ...
          <F label="Link Autovit">
            <input
              value={form.autovit_url ?? ""}
              onChange={(e) => setForm({ ...form, autovit_url: e.target.value })}
      ...
      <section className="surface-card p-6">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          ...
          <F label="Capacitate motor (cm³)">
            <input
              type="number"
              value={form.engine_size}
              onChange={(e) => setForm({ ...form, engine_size: Number(e.target.value) })}
              className={inputCls}
            />
          </F>
          <F label="Putere (CP)">
      ...
      <section className="surface-card p-6">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Încarcă direct de pe dispozitiv (stocare Cloudflare R2) sau adaugă URL-uri externe. Prima
          imagine este cea principală.
        </p>
        <div className="mt-3">
        ...
        <div className="mt-4 flex gap-2">
          ...
          <button
            type="button"
            onClick={addImage}
            className="inline-flex items-center gap-2 rounded-xl bg-foreground px-4 py-2.5 text-sm text-background"
          >
        ...
        <ul className="mt-4 space-y-2">
            ...
            >
              <div className="flex flex-col">
                <button
                  ...
                  onClick={() => move(i, -1)}
                  disabled={i === 0}
                  className="h-5 text-muted-foreground disabled:opacity-30"
                >
                  ↑
                </button>
          ...
          {images.length === 0 && (
            <li className="text-sm text-muted-foreground">Nicio imagine adăugată.</li>
          )}
        </ul>
      </section>
      ...
      <div className="flex items-center justify-end gap-2">
        <button
          type="button"
          onClick={() => navigate({ to: "/admin/masini" })}
        ...
        <button
          disabled={saving}
          type="submit"
          className="rounded-xl bg-foreground px-5 py-2.5 text-sm font-medium text-background disabled:opacity-60"
        >
